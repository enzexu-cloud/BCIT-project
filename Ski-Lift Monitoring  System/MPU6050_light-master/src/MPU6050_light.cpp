...
void MPU6050::update(){
  ...
  // Correctly wrap X and Y angles
  angleX = wrap(filterGyroCoef * (angleX + gyroX * dt) + (1.0 - filterGyroCoef) * angleAccX, 180.0);
  angleY = wrap(filterGyroCoef * (angleY + sgZ * gyroY * dt) + (1.0 - filterGyroCoef) * angleAccY, 90.0);
  angleZ = wrap(angleZ + gyroZ * dt, 90.0);
 ...
 */
 ...
 void MPU6050::calibrateDriftRateZ(unsigned long calibrationTime) {
  float initialAngleZ = angleZ;
  delay(calibrationTime); // Wait for the calibration period
  float finalAngleZ = angleZ;
  driftRateZ = (finalAngleZ - initialAngleZ) / (calibrationTime * 1e-3); // Drift rate in degrees per second
}
...
void MPU6050::update() {
  ...
  static float accXFiltered = 0, accYFiltered = 0, accZFiltered = 0;
  const float alpha = 0.02; // Low-pass filter coefficient (adjust as needed)

  accXFiltered = alpha * accX + (1.0 - alpha) * accXFiltered;
  accYFiltered = alpha * accY + (1.0 - alpha) * accYFiltered;
  accZFiltered = alpha * accZ + (1.0 - alpha) * accZFiltered;

  // Estimate tilt angles using filtered accelerometer data
  float sgZ = accZFiltered < 0 ? -1 : 1; // Allow one angle to go from -180 to +180 degrees
  angleAccX = atan2(accYFiltered, sgZ * sqrt(accZFiltered * accZFiltered + accXFiltered * accXFiltered)) * RAD_2_DEG; // [-180,+180] deg
  angleAccY = -atan2(accXFiltered, sqrt(accZFiltered * accZFiltered + accYFiltered * accYFiltered)) * RAD_2_DEG;      // [-90,+90] deg

  unsigned long Tnew = millis();
  float dt = (Tnew - preInterval) * 1e-3;
  preInterval = Tnew;

  // Apply noise threshold to gyro Z
  if (gyroZ > -2.0 && gyroZ < 2.0) {
      gyroZ = 0;
  }
  ...
  // Update yaw angle with drift compensation
  angleZ = wrap(angleZ + gyroZ * dt - driftRateZ * dt, 180.0);
}
...
void MPU6050::update() {
    ...
    preInterval = Tnew;
      ...
      // Apply noise threshold to gyro X Y Z
      if (gyroZ > -2.0 && gyroZ < 2.0) {
        gyroZ = 0;
      }   
...
/* V1.0.0
 void MPU6050::update(){
   ...
   if(gyroZ>-2.0 && gyroZ<2.0) {gyroZ=0;}
