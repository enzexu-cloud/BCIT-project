...
//172.20.10.13
// At the top
const char* chairName = "Chair 2";  // Change to "Chair 3" for Chair 3
...
String generateEmergencyMessage(float angleX, float angleY, float angleZ) {
  ...
  String msg = "EMERGENCY|ID:" + id + "|SRC:" + String(chairName) +
...
void loop() {
  ...
  String data = "ANGLE|NAME:" + String(chairName) +
              "|R:" + String(angleX, 2) + 
              "|P:" + String(angleY, 2) + 
              "|Y:" + String(angleZ, 2);
...
void softRest(){
  // Check for incoming UDP commands (e.g., RESET)
...
if (packetSize > 0) {
  ...
  if (len > 0) {
    ...
    Serial.println("UDP command received: " + command);
